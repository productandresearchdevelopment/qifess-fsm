...
<script>
  var Grids = function() {
    ...
    me.init = function() {
      ...
      me.store = me.httpStore('{{ route('technicianvendor.data') }}', [{
          name: 'id',
          type: 'int'
        },
        {
      ...
      me.menus = Ext.create('Ext.menu.Menu', {
        items: [
          @if ($user->hasRoute('technicianvendor.push'))
            {
              text: 'Create',
              iconCls: 'icon-add',
          ...
          @if ($user->hasRoute('technicianvendor.delete'))
            {
              text: 'Delete',
              iconCls: 'icon-remove',
              handler: function() {
                ...
                if (data.length) {
                  Ext.ajaxConfirm('Remove Name Vendor', {
                    mask: me.grid,
                    url: '{{ route('technicianvendor.delete') }}',
                    params: {
                      '_method': 'DELETE',
                      '_token': '{{ csrf_token() }}',
          ...
          @endif
          @if ($user->hasRoute('technicianvendor.restore') || $user->hasRoute('technicianvendor.forcedelete'))
            {
              text: 'Trashed',
              iconCls: 'icon-trash',
              menu: {
                items: [
                  @if ($user->hasRoute('technicianvendor.restore'))
                    {
                      text: 'Restore',
                      ...
                      handler: function() {
                        ...
                        if (recs.length) {
                          Ext.ajaxConfirm('Restore Name Vendor', {
                            mask: me.grid,
                            url: '{{ route('technicianvendor.restore') }}',
                            params: {
                              '_method': 'PUT',
                              '_token': '{{ csrf_token() }}',
                  ...
                  @if ($user->hasRoute('technicianvendor.forcedelete'))
                    {
                      text: 'Forever Remove',
                      iconCls: 'icon-remove',
                      handler: function() {
                        ...
                        if (recs.length) {
                          Ext.Msg.confirm('Confirm',
                            ...
                            function(btn) {
                              if (btn === 'yes') {
                                Ext.Ajax.request({
                                  url: '{{ route('technicianvendor.forcedelete') }}',
                                  method: 'DELETE',
                                  params: {
                                    '_method': 'DELETE',
                                    '_token': '{{ csrf_token() }}',
